...
<head>
  ...
  <style>
    h1,h2,.font-display{font-family:'Bricolage Grotesque',system-ui,sans-serif;}
...
<div class="blob w-96 h-96 bg-blue-600/20 -top-24 -left-24" style="animation-delay:0s"></div>
<div class="blob w-80 h-80 bg-violet-600/15 bottom-0 right-0" style="animation-delay:-3s"></div>

<div class="relative w-full max-w-[420px]">
  <!-- Back to landing -->
  <a href="<?= url('/') ?>" class="inline-flex items-center gap-1.5 text-slate-500 hover:text-slate-300 text-xs font-medium mb-6 transition-colors">
    <i data-lucide="arrow-left" class="w-3.5 h-3.5"></i> Kembali ke beranda
  </a>

  <div class="text-center mb-8">
    <a href="<?= url('/') ?>" class="inline-block">
      <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center mx-auto mb-5 shadow-[0_0_40px_rgba(37,99,235,0.4)]">
        <i data-lucide="graduation-cap" class="w-8 h-8 text-white"></i>
      </div>
    </a>
    <h1 class="font-display font-bold text-white text-3xl tracking-tight">SiAkad SD</h1>
    <p class="text-slate-500 mt-2 text-sm">Sistem Informasi Akademik</p>
  </div>
  ...
  <div class="card-login rounded-3xl border border-white/8 bg-white/[0.04] backdrop-blur-2xl p-8 shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
    ...
    <form method="POST" action="<?= url('/login') ?>" class="space-y-4">
      ...
      <label class="flex items-center gap-3 cursor-pointer py-1">
        <input type="checkbox" name="remember_me" value="1" class="rounded accent-blue-500 w-4 h-4">
        <span class="text-sm text-slate-400">Ingat saya selama 30 hari</span>
      </label>

      <button type="submit"
              class="w-full py-3 px-4 bg-blue-600 hover:bg-blue-500 text-white font-semibold rounded-xl
    ...
    <div class="mt-6 pt-6 border-t border-white/8">
      ...
      <div class="grid grid-cols-3 gap-2">
        <button onclick="fillDemo('admin@sekolah.sch.id')" class="py-2.5 px-2 rounded-xl bg-violet-500/15 border border-violet-500/25 text-violet-400 text-xs font-semibold hover:bg-violet-500/25 transition-colors">🛠 Admin</button>
        <button onclick="fillDemo('budi@sekolah.sch.id')"  class="py-2.5 px-2 rounded-xl bg-blue-500/15 border border-blue-500/25 text-blue-400 text-xs font-semibold hover:bg-blue-500/25 transition-colors">👨‍🏫 Guru</button>
        <button onclick="fillDemo('ahmad@sekolah.sch.id')" class="py-2.5 px-2 rounded-xl bg-emerald-500/15 border border-emerald-500/25 text-emerald-400 text-xs font-semibold hover:bg-emerald-500/25 transition-colors">👦 Murid</button>
      </div>
      <p class="text-slate-700 text-xs text-center mt-2">Password: <code class="bg-white/8 px-1.5 py-0.5 rounded font-mono text-slate-500">password</code></p>
    </div>
  </div>

  <p class="text-center text-slate-700 text-xs mt-6">SiAkad SD &copy; <?= date('Y') ?> &mdash; Tahun Ajaran <?= TAHUN_AJARAN ?></p>
</div>
...
if (localStorage.getItem('theme')==='dark'||(!(localStorage.getItem('theme'))&&window.matchMedia('(prefers-color-scheme: dark)').matches)) h.classList.add('dark');
</script>
</body>
</html>
